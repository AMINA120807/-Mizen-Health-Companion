...
export default function HistoryTab() {
  ...
  if (historyStats.daysLogged === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-in fade-in">
        ...
        <p className="text-gray-500 dark:text-gray-400 font-medium text-center">{t('history.noData')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full glass rounded-2xl p-6 shadow-md animate-in fade-in slide-in-from-bottom-4 duration-700 relative overflow-hidden">
      ...
      <h2 className="font-heading text-2xl font-bold text-gray-800 dark:text-gray-200 mb-8 text-center">{t('history.title')}</h2>
      
      <div className="grid grid-cols-2 gap-4 text-center mb-8">
        <div className="bg-white/60 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm hover-lift">
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium mb-2">{t('history.avgDailyCalories')}</p>
          <p className="font-heading font-bold text-3xl text-gray-900 dark:text-gray-100">{historyStats.avgCalories.toFixed(0)}</p>
        </div>
        <div className="bg-white/60 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm hover-lift">
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium mb-2">{t('history.avgDailyGL')}</p>
          <p className="font-heading font-bold text-3xl text-accent">{historyStats.avgGL.toFixed(1)}</p>
        </div>
      </div>
      
      <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-8 bg-gray-50 dark:bg-slate-800/50 py-2 rounded-lg">
        {t('history.basedOn')} <span className="font-bold text-gray-700 dark:text-gray-300">{historyStats.daysLogged}</span> {t('history.daysOfData')}
      </p>
