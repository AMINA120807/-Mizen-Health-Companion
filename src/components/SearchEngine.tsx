    ...
    <div className="w-full glass rounded-2xl p-6 mb-8 transition-all">
      <div className="relative">
        <input
          ...
          className="w-full p-4 pl-12 border border-gray-200 dark:border-emerald-800/50 rounded-xl mb-6 bg-white dark:bg-emerald-950/40 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
          dir="auto"
        />
      ...
      <div className="space-y-3">
        {results.length > 0 ? (
          results.map(food => (
            <div key={food.id} className="p-4 bg-white dark:bg-emerald-950/40 border border-gray-100 dark:border-emerald-900/50 rounded-xl flex justify-between items-center hover-lift">
              <div className="flex-1">
                <div className="font-heading font-semibold text-lg text-gray-900 dark:text-emerald-50">{getFoodName(food)}</div>
                <div className="text-sm text-gray-500 dark:text-emerald-200/60 mb-1">
          ...
          <div className="text-gray-500 dark:text-emerald-200/60 text-center py-6 bg-gray-50 dark:bg-emerald-900/20 rounded-xl border border-dashed">{t('search.noResults')}</div>
        ) : null}
      </div>
    </div>
  );
