    ...
    <div className="w-full transition-all fade-in print-container">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 print-hide">
        <div>
          ...
          <p className="text-gray-500 dark:text-emerald-200/60 text-sm">
            {t('weekly.subtitle')}
          </p>
        </div>
        <button
      ...
      {activeDay !== null && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 print-hide">
          <div className="bg-white dark:bg-emerald-950/40 rounded-2xl w-full max-w-md p-6 shadow-2xl relative">
            ...
            <h3 className="font-bold text-lg mb-4 text-gray-900 dark:text-emerald-50">
            ...
            <div className="relative mb-4">
              <input
                ...
                className="w-full p-3 border border-gray-200 dark:border-emerald-800/50 rounded-xl bg-gray-50 dark:bg-emerald-900/20 focus:outline-none focus:ring-2 focus:ring-primary"
                dir="auto"
              />
            </div>
              ...
              {searchResults.length > 0 ? (
                searchResults.map(food => (
                  <button
                    ...
                    className="w-full text-left px-4 py-3 hover:bg-primary/5 rounded-xl border border-gray-100 dark:border-emerald-900/50 flex justify-between items-center transition-colors"
                  >
                    <span className="font-medium text-gray-900 dark:text-emerald-50">{getFoodName(food)}</span>
                    <span className="text-sm text-gray-500 dark:text-emerald-200/60 bg-gray-100 dark:bg-emerald-900/30 px-2 py-0.5 rounded">{food.calories_per_100g} kcal</span>
                  </button>
                ))
              ) : query.trim() ? (
                <p className="text-center text-gray-500 dark:text-emerald-200/60 py-4">{t('search.noResults')}</p>
              ) : null}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 print-grid">
        {plan.map((dayPlan) => (
          <div key={dayPlan.dayId} className="bg-white dark:bg-emerald-950/40 border border-gray-200 dark:border-emerald-800/50 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col h-full print-card">
            {/* Day Header */}
            <div className="bg-primary/5 border-b border-gray-100 dark:border-emerald-900/50 p-4 flex justify-between items-center print-card-header">
              <h3 className="font-bold text-gray-900 dark:text-emerald-50">{daysOfWeek[dayPlan.dayId]}</h3>
              <button 
                ...
                dayPlan.meals.map((meal, index) => (
                  <div key={`${meal.id}-${index}`} className="flex justify-between items-start group">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="font-medium text-sm text-gray-900 dark:text-emerald-50 leading-tight">{getFoodName(meal)}</p>
                        <p className="text-xs text-gray-500 dark:text-emerald-200/60">{meal.calories_per_100g} kcal/100g</p>
                      </div>
                    </div>
                    <button 
