      ...
      <div className="mb-8">
        ...
        <div className="relative">
          <input
            ...
            className="w-full p-3 border border-gray-200 dark:border-emerald-800/50 rounded-xl bg-white dark:bg-emerald-950/40 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary"
            dir="auto"
          />
          {searchResults.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white dark:bg-emerald-950/40 border border-gray-100 dark:border-emerald-900/50 rounded-xl shadow-lg">
              {searchResults.map(food => (
                <button
                  key={food.id}
                  onClick={() => addIngredient(food)}
                  className="w-full text-left px-4 py-3 hover:bg-gray-50 dark:bg-emerald-900/20 border-b border-gray-50 last:border-0 flex justify-between items-center"
                >
                  <span className="font-medium text-gray-900 dark:text-emerald-50">{getFoodName(food)}</span>
                  <span className="text-sm text-gray-500 dark:text-emerald-200/60">{food.calories_per_100g} kcal / 100g</span>
                </button>
              ))}
            </div>
          )}
      ...
      {ingredients.length > 0 && (
        <div className="mb-8 space-y-3">
          ...
          {ingredients.map((item, index) => (
            <div key={index} className="flex items-center gap-3 bg-white dark:bg-emerald-950/40 p-3 rounded-xl border border-gray-100 dark:border-emerald-900/50 shadow-sm">
              <div className="flex-1 font-medium text-gray-900 dark:text-emerald-50 truncate">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-20 p-2 border border-gray-200 dark:border-emerald-800/50 rounded-lg text-center focus:ring-2 focus:ring-primary focus:outline-none"
                />
                <span className="text-gray-500 dark:text-emerald-200/60 text-sm">g</span>
              </div>
              <button 
          ...
          <div className="text-right text-sm text-gray-600 dark:text-emerald-200/60 font-medium pt-2 border-t border-gray-100 dark:border-emerald-900/50">
        ...
        <div className="space-y-4 mb-8 bg-primary/5 p-5 rounded-xl border border-primary/10">
          <div>
            ...
            <input
              ...
              className="w-full p-3 border border-gray-200 dark:border-emerald-800/50 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none"
              dir="auto"
            />
          </div>
          <div>
